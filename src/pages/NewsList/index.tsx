...
import { doGetNewsList } from "../../store/actions/newsActions";
import "./style.css";
    ...
    return (
        <Layout>
            ...
            <div id="page-content-wrapper">
                <Header />
                <div className="newsList container-fluid">
                    <div className="row" >
                        {newsList?.length > 0 && (
                            newsList.map((news) => (
                                <div className="col-md-6 mb-5" key={news?.id}>
                                    <div className="card h-100">
                                        <div className="card-body">
                                            ...
                                            <p className="card-text">{news?.description?.substring(0, 65) + "..."}</p>
                                        </div>
                                        <div className="card-footer">{new Date(news?.date)?.toDateString()}
                                            <Button type="button" className="action-button" onClick={() => { handleViewModal(news) }}>
                ...
                {viewModal &&
                    (
                        <ViewModal show={viewModal} onHide={handleCloseViewModal} newsData={newsDetails} />
                    )
                }
            </div>
